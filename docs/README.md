...
- [Project Graph](project_graph.md): text diagrams for runtime dependencies, MainWindow wiring, canvas/layer flow, camera workflows, package import/export, and settings.

## Current Product Shape

Camera Map View is a PyQt6 desktop diagramming tool for camera map layouts. The current UI is organized around:

- A fixed left Control Panel that combines layout management and camera inventory.
- A central `MapCanvas` with pan/zoom, background map support, grid rendering, camera items, and drawing annotations.
...
The app starts in `Pan` mode to prevent accidental camera or drawing edits. Object interaction happens only after switching to `Select`.
...
Each layout owns its cameras, drawing annotations, layer stack, background map path, grid size, canvas dimensions, and background scale.
...
Import creates a new layout instead of overwriting the current one.
